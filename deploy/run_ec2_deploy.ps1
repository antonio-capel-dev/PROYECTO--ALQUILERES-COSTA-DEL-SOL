...
try {
    ...
    Write-Host "  (No se pudieron ajustar permisos - continua si ya son correctos)" -ForegroundColor Yellow
}

$Remote   = "${SshUser}@${ServerIP}"
$SshArgs  = @("-i", $PemPath, "-p", $SshPort, "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=15")
$ScriptLocal = "deploy\devops_pro_deploy.sh"

Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  CostaSol - Deploy remoto EC2 (Senior DevOps)"                   -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
...
& scp @scpArgs $ScriptLocal "${Remote}:/tmp/devops_pro_deploy.sh"
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
& ssh @sshArgs $Remote "chmod +x /tmp/devops_pro_deploy.sh && bash /tmp/devops_pro_deploy.sh"
